...
/// Terminal height (rows) at which the base parameters feel right.
const REFERENCE_HEIGHT: f64 = 60.0;
...
impl ScrollPhysics {
    pub fn new() -> Self {
        Self {
            ...
            air_drag: BASE_AIR_DRAG,
        }
    }

    /// Update physics parameters for the given terminal height.
    /// Smaller terminals get stronger friction/drag so animations complete faster.
    pub fn configure(&mut self, terminal_height: usize) {
        let scale = REFERENCE_HEIGHT / (terminal_height as f64);
        self.friction = BASE_FRICTION * scale;
        self.air_drag = BASE_AIR_DRAG * scale;
    }
    ...
    pub fn impulse(&mut self, target_rows: f64) {
        let impulse = self.impulse_for_distance(target_rows.abs());
    ...
    pub fn tick(&mut self, dt: f64) -> isize {
        ...
        let friction_force = -sign * self.friction;
        let drag_force = -self.velocity * self.air_drag;
    ...
    fn impulse_for_distance(&self, target_distance: f64) -> f64 {
        ...
        let mut hi = target_distance * self.air_drag * 4.0;

        for _ in 0..64 {
            let mid = (lo + hi) / 2.0;
            if self.analytical_distance(mid) < target_distance {
                lo = mid;
            } else {
                hi = mid;
            }
    ...
    fn analytical_distance(&self, v0: f64) -> f64 {
        ...
        let a = self.friction / self.air_drag; // F/D
        (v0 - MIN_VELOCITY) / self.air_drag
            - a / self.air_drag * ((v0 + a) / (MIN_VELOCITY + a)).ln()
    }
}
...
mod tests {
    ...
    fn analytical_distance_sanity() {
        ...
        assert!((physics.analytical_distance(0.0) - 0.0).abs() < f64::EPSILON);
        assert!((physics.analytical_distance(MIN_VELOCITY) - 0.0).abs() < f64::EPSILON);
        // Monotonically increasing above MIN_VELOCITY
        let mut prev = 0.0;
        for i in 2..=100 {
            let v0 = i as f64;
            let d = physics.analytical_distance(v0);
            assert!(
                d > prev,
    ...
    fn impulse_for_known_distances() {
        ...
        for target in [5.0, 10.0, 20.0, 50.0, 100.0] {
            let v0 = physics.impulse_for_distance(target);
            let actual = physics.analytical_distance(v0);
            assert!(
    ...
    /// Count how many ticks until the simulation completes.
    fn tick_count(physics: &mut ScrollPhysics) -> usize {
        let dt = 1.0 / 120.0;
        let mut count = 0;
        for _ in 0..10000 {
            if !physics.is_active() {
    ...
    #[test]
    fn small_terminal_completes_faster() {
        // Half-page scroll on a small terminal (20 rows → 10 row scroll)
        let mut small = ScrollPhysics::new();
        small.configure(20);
        small.impulse(10.0);
        let small_ticks = tick_count(&mut small);

        // Half-page scroll on a large terminal (60 rows → 30 row scroll)
        let mut large = ScrollPhysics::new();
        large.configure(60);
        large.impulse(30.0);
        let large_ticks = tick_count(&mut large);

        // The small terminal animation should not take longer than the large one.
        assert!(
            small_ticks <= large_ticks,
            "small terminal should complete faster or equally: small={small_ticks}, large={large_ticks}"
        );
    }
}
