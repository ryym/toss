//! Parses text on terminal and recognizes control characters in it.
...
/// Split text into control sequences and plain text segments.
pub fn parse_text(text: &str) -> impl Iterator<Item = Text<'_>> {
    TextIter::new(text)
}

/// A segment of parsed terminal text.
#[derive(Debug, PartialEq)]
pub enum Text<'t> {
    /// An ANSI control sequence or C0 control character.
    Control(&'t str),
    /// Printable text without control sequences.
    Plain(&'t str),
}
